...
class TestCheckPerms:
    """Test the _check_perms helper function."""

    def test_check_perms_returns_true_for_valid_perm(self):
        from musicbot.cogs.music import _check_perms
        from conftest import make_mock_bot, make_mock_interaction

        bot = make_mock_bot()
        interaction = make_mock_interaction()

        # Mock permission group with instaskip=True
        perm_group = MagicMock()
        perm_group.instaskip = True
        bot.permissions.for_user.return_value = perm_group

        assert _check_perms(bot, interaction, "instaskip") is True

    def test_check_perms_returns_false_for_denied_perm(self):
        from musicbot.cogs.music import _check_perms
        from conftest import make_mock_bot, make_mock_interaction
        ...
        perm_group = MagicMock()
        perm_group.instaskip = False
        bot.permissions.for_user.return_value = perm_group

        assert _check_perms(bot, interaction, "instaskip") is False

    def test_check_perms_defaults_to_true_for_unknown_perm(self):
        """If the permission attribute doesn't exist, default to True."""
        from musicbot.cogs.music import _check_perms
        from conftest import make_mock_bot, make_mock_interaction
        ...
        perm_group = MagicMock(spec=[])  # empty spec = no attributes
        bot.permissions.for_user.return_value = perm_group

        # _check_perms uses getattr with default True
        result = _check_perms(bot, interaction, "nonexistent_perm")
